...
- Read-only Rust LSP intelligence is available through five tools (`lsp_status`, `document_symbols`, `goto_definition`, `find_references`, `document_diagnostics`) via agent-side rust-analyzer with a constrained profile (`cargo.noDeps=true`, no build scripts/proc macros/checkOnSave). Paths are project-relative; columns are 1-based Unicode scalars; external/dependency results are omitted. Open documents refresh from bounded workspace file content using full-text sync only. Diagnostics are bounded `publishDiagnostics` feedback with explicit freshness/timeout state, not a replacement for Cargo validation. Workspace symbols, editor-style incremental sync, multi-language support, and LSP write operations are not first-class yet.
