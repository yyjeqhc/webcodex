...
- [x] Keep the recommended server bootstrap path centered on `webcodex-cli server init`, `webcodex-cli server install-service`, and `webcodex-cli server status`.
- [x] Add a separate client-side setup/enroll flow for user API tokens and agent tokens.
- [x] Add client-side `webcodex-cli agent install-service` and `webcodex-cli agent status` so binary deployment does not require hand-written systemd units.
