...
export function DiagnosticsPanel({ state, onState }: { state: DesktopState; onState: (state: DesktopState) => void }) {
  ...
  return <section className="diagnostics-panel" aria-label={s("Diagnostics Center")}>
    ...
    {data && <>
      ...
      <section className="shell-subsection" aria-labelledby="tool-request-tracing-title">
        <h3 id="tool-request-tracing-title">{s("Tool Request Tracing")}</h3>
      ...
      <section className="shell-subsection" aria-label={s("Open Runtime Console")}>
      ...
      <section className="shell-subsection" aria-label={s("Copy Diagnostic Report")}>
      ...
      <section className="shell-subsection"><h3>{s("Latest observed Window")}</h3><ContinuationFacts value={data.report.last_webcodex_call} observedAt={data.observed_at_ms} /></section>
