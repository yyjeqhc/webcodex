...
async fn touch_runner_refreshes_stale_client_back_to_online() {
    ...
    registry
        .set_last_seen_for_test("oe", now_ts() - RUNNER_ONLINE_WINDOW_SECS - 1)
        .await;
    let stale = registry.get_runner_view("oe").await.unwrap();
    assert!(!stale.connected);
    assert_eq!(stale.status, "stale");

    // A keepalive touch must bring it back online.
    registry.touch_runner("oe", "inst").await.unwrap();
    let fresh = registry.get_runner_view("oe").await.unwrap();
    assert!(fresh.connected);
    assert_eq!(fresh.status, "online");

    // Unknown client_id is a clear error and does not mutate state.
    assert!(registry.touch_runner("nope", "inst").await.is_err());
}

#[tokio::test]
async fn touch_runner_rejects_stale_instance_and_accepts_active() {
    ...
    let before = registry.get_runner_view("oe").await.unwrap().last_seen;
    ...
    // last_seen for B.
    let err = registry.touch_runner("oe", "inst-a").await.unwrap_err();
    assert!(
        err.contains("no longer the active instance"),
        "error was: {err}"
    );
    let after_a = registry.get_runner_view("oe").await.unwrap().last_seen;
    assert_eq!(
        after_a, before,
        "stale instance touch must not refresh active last_seen"
    );
    // A stale instance must not resurrect the client to online either.
    let view_after_a = registry.get_runner_view("oe").await.unwrap();
    assert!(view_after_a.connected);

    // Active instance B's keepalive succeeds and refreshes last_seen.
    registry.touch_runner("oe", "inst-b").await.unwrap();
    let after_b = registry.get_runner_view("oe").await.unwrap().last_seen;
    assert!(
        after_b > before,
    ...
    assert!(registry.touch_runner("oe", "").await.is_err());
}
