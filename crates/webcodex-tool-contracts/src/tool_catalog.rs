...
pub const TOOL_RECOMMENDED_FLOWS: &[ToolRecommendedFlow] = &[
    ...
    ToolRecommendedFlow {
        ...
        manifest_purpose: "WebCodex-owned cross-repository workflow, not AGENTS.md policy. work_on_project may return sparse goal_context for active Goals explicitly correlated to the exact authorized Workflow Session. Reuse one exact candidate by calling get_goal and present_goal_plan, or explicitly choose among multiple candidates before those exact reads; never auto-select or infer from Project, Window, title, or recency. For ordinary new multi-step/cross-turn work without reusable active Goal context, call prepare_goal_workflow with the exact Workflow Session, bounded completion_conditions/steps and optional explicit controller Agent, then present_goal_plan. available=false is unavailable evidence, not proof of zero active Goals. prepare_goal_workflow is durable admission only: Host carrier setup/readiness remains separate. If auto-resume is required and the chosen controller is not already production-ready, use the existing agent_continuation_setup flow. Use checkpoint_goal at recovery-worthy boundaries; finish_coding_task returns the same sparse active Goal representation as goal_follow_up. After fresh validation/review explicitly complete all steps and update_goal. Low-level create_goal and associate_goal_workflow_session remain available for advanced composition. Tiny reads/trivial edits do not require Goal setup.",
        tools: &[
