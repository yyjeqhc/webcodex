    ...
    adaptive_runtime_direct(
        permission_risk(
            model_spec(
                ...
                "Small/local exact edits use a transactional structured option: use ONE change per file with multiple edits. Globally unique edits may omit expected_read_revision; occurrence or line_scope requires expected_read_revision; revisions fence whole-file snapshots; model input never needs a digest. For the same old_text in one explicit bounded file when exact cardinality is known, use replace_exact expected_match_count=N (1..=1024), optionally scoped, never occurrence. If count/range is uncertain, optional dry_run; if obvious apply directly—dry_run is not ritual. Batches are preflighted transactionally; conflicts fail closed; Runner rechecks source before mutation. Successful change_summary/changed_files/resolved_matches confirm mechanical scope; do not diff only to recount. Summary is not semantic review: use show_changes, git_diff_hunks, or git_review_summary. On stale state use one parser-ready read_files recovery call; inspect the resulting diff when semantic review is needed; validate the final source.",
