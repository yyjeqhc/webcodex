...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    adaptive_runtime_direct(
        model_spec(
            ...
            "Run one native executable with structured literal argv; prefer this over run_shell unless shell grammar or a short related command chain is required. Windows batch shims use the bounded Runner-owned quoting contract. Persistent shell is only for retained same-process or named-SSH state, not command count. Long work stays the same execution and remains Runner-owned; timeout_secs defaults to 60s and clamps at 7 days. On Job handoff retain exact job_id/continuation, continue independent work, and passive Job attention may surface transitions. Use observe_jobs only for logs/details/recovery; use wait_for_job_terminal only when terminal outcome is a true dependency and no independent work remains. Use run_detached_process only when the native child must survive Runner restart/upgrade/stop/replacement; duration alone is not a reason to detach.",
        ).with_gpt_action_description("Run one native executable with literal argv. On Job handoff retain exact identity, continue independent work, use observe_jobs only for details/recovery, and wait_for_job_terminal only when terminal outcome blocks progress. Detach only to survive Runner restart/upgrade.")
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Explicit logs/details/recovery continuation for known exact Jobs; not the default Job-handoff step. Continue by job_id; do not call list_jobs first. Prefer observation_ref, or pass observation_token unchanged as after_observation_token; raw job_id/token are recovery fallback. No token gives an immediate baseline; no wait_secs gives an immediate observation. Passive Job attention may surface transitions. With tokens, bounded wait_secs uses wake_on=change by default; use a bounded wait_secs with wake_on=terminal only when useful progress is blocked on terminal outcome. If independent work remains, continue it—do not poll for visibility. terminal wakes on any terminal Job; all_terminal waits for all. Item errors return immediately; timeout may include changed=true; unknown/expired refs fail only that item. summary_only may compact proven successful validation logs. Never launches, retries, stops, or subscribes.",
        ).with_gpt_action_description("Observe a known exact Job only for logs/details/recovery. Prefer the returned continuation; do not list first or poll mechanically. observation tokens/refs are cursors, never launch or retry authority."),
        80,
    ),
    adaptive_runtime_direct(
        model_spec(
