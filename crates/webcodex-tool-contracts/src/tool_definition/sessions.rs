...
pub(super) const DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Canonical bootstrap for ordinary coding/review. Prefer Server-issued project_ref; project or client_id+path work. Omit session_id for a fresh Workflow Session; it does not imply a fresh model context. Exact resume requires an active accessible Session and never guesses prior Session. Exact resume may return sparse owner-scoped goal_context for active Goals explicitly correlated to that Session; reuse one exact candidate or explicitly choose among multiple. This projection grants no authority and never selects/binds a Goal. For a fresh or uncertain model context, request missing project.instructions/webcodex.workflow via context_request. Runtime re-observes instruction files. guidance_profile is model guidance only. project_ref is principal-scoped convenience, not authority; each use reauthorizes the Project. include_extension_catalog controls bounded Skills/Plugins. Checkout does not require Git; mode=worktree creates an isolated worktree from an exact Git base without widening Project authority.",
