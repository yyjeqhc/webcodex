...
2. 把 OAuth2 和 GPT Action/MCP setup 作为 first-class entry points 持续完善文档。
...
5. 先做 policy-first operations capabilities，再做危险 remediation tools。
6. 先设计 provider abstractions，再实现 LSP、Tree-sitter、systemd、Docker 或 messaging integrations。
7. 把 design documents 当作 architecture constraints，而不是 marketing text。
