...
All endpoints are `POST` and protected by Bearer auth. Use the bootstrap/admin
token or an admin-scoped personal API token for this admin/debug surface. The
endpoints perform no write operations.
