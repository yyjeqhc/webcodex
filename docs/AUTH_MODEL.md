...
`webcodex connect <server> --auth oauth` uses its own explicit closed delegation set: runtime/project/job/Computer scopes needed by the hosted MCP workflow, intersected with the Server's advertised registry. It intentionally excludes `account:manage`, Agent/admin authority, protocol-only `offline_access`, and any future permission until that hosted-connect list is explicitly reviewed. The persisted allow-list is stable across reconnects. An active matching client is reused without redisclosing its secret; a missing or revoked client is rotated to new credentials. The Runner uses a separately issued `wc_agent_*` credential; OAuth access tokens remain forbidden on Agent transport.
