...
`webcodex connect <server> --auth oauth` is the ordinary hosted shared-key OAuth bridge. The OAuth client is owned by the direct shared key's SHA-256 group hash, and authorization codes, access tokens, and refresh tokens carry that same `shared_key_hash` subject binding. The bridge delegation ceiling is not the global OAuth registry: it is exactly the direct shared-key model-facing set (`runtime:read`, `project:read`, `project:write`, `job:run`, `computer:read`, `computer:control`). `account:manage`, `admin`, every `agent:*` transport scope, Computer launch/display/pointer/clipboard permissions, and future scopes are excluded. `offline_access` remains protocol-only. A shared-key-owned browser authorization additionally requires the corresponding Runner group to remain connected. Reconnect reuses an exact active client without widening or redisclosing its secret; replacement after revocation/missing state preserves the previously persisted ceiling. The Runner continues using the direct shared key, and OAuth access tokens remain forbidden on Agent transport. `--auth managed-oauth` retains the separate managed-user flow.
