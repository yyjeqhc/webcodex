...
`webcodex connect <server> --auth oauth --oauth-redirect-uri <exact-callback>` is the ordinary ChatGPT OAuth path for hosted connect. It uses the same `wck_*` shared-key identity as the Runner, provisions a server-side OAuth client bound to that shared-key hash, and asks for the shared key only on WebCodex's own browser authorize page. ChatGPT receives the OAuth client credentials/tokens, never the shared key. The bridge ceiling exactly matches direct shared-key model-facing authority: `runtime:read`, `project:read`, `project:write`, `job:run`, `computer:read`, and `computer:control`; it excludes `account:manage`, `admin`, every `agent:*` scope, Computer launch/display/pointer/clipboard scopes, and future permissions. Refresh tokens preserve the same shared-key subject binding, and OAuth access tokens remain invalid on Agent transport. Reconnect reuses a matching active client without widening its allow-list or redisclosing the secret; missing/revoked clients rotate under the previously persisted ceiling.
