...
macOS currently keeps `computer_pointer_control=false` and has no production pointer dispatch path, but the private M4a preflight substrate is implemented for later effect wiring. For an exact revalidated unrotated display, source-pixel coordinates map independently per axis as `global_origin + (source_coordinate / source_extent) * global_bounds_extent`, using the current `CGDisplayBounds` rectangle and never encoded JPEG dimensions, `CGDisplayPixelsWide/High`, a hard-coded Retina scale, or integer rounding. The Runner then revalidates the same display identity/source geometry and rereads bounds/rotation; any placement or rotation change is `stale_display`, while non-finite/empty bounds and any non-zero rotation fail closed. The read-only shared-input preflight uses the current login session's combined Quartz event-source state, checks all 32 Quartz mouse-button slots for move/click, additionally rejects Shift/Control/Option/Command for click, and uses `CGPreflightPostEventAccess` without requesting permission. Bounds, rotation, mapped global targets, scale, and native display ids remain private and are not model/audit output. No mouse event is created or posted in this slice.
