...
macOS prepares one `CombinedSessionState` `CGEventSource` plus the complete fixed event set before generation spend: exactly one `MouseMoved` for move, or one `MouseMoved` plus one `LeftMouseDown` and one `LeftMouseUp` for click. Dispatch posts only at the HID event tap and has no warp, NSEvent injection, AppleScript, fallback, repair, or retry. After `MouseMoved`, a fresh public Quartz cursor-location observation (`CGEventCreate(NULL)` plus `CGEventGetLocation`) must exactly equal the prepared fractional global target; there is no epsilon or rounding. Click then performs a second fresh held-state proof before posting down/up. Success after `LeftMouseUp` additionally requires the cursor still exactly at the target and the shared left-button state to be up. Quartz posting exposes no insertion-count acknowledgement, so any readback failure, target mismatch, interruption, dirty second click preflight, or failed final proof after a post is `outcome_unknown`; button events are not attempted when the move proof or second held-state proof fails.
