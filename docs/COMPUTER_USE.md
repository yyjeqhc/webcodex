...
`computer_snapshot_display` captures exactly one previously discovered display; there is no virtual-desktop mosaic, caller region, global coordinate, pointer/click input, activation, or fallback. On Windows the Runner re-enumerates native displays and requires an exact private monitor-interface identity plus unchanged display-relative source geometry. On macOS it binds the public ColorSync display UUID and bounded CoreGraphics device identity to the discovery-time native display id and source pixel geometry, rejects ambiguous identity, and uses the no-UI Screen Recording preflight before exact-display capture. Both implementations verify captured pixel geometry and revalidate identity again after capture so a hotplug/replacement race discards the bytes rather than accepting the wrong display. `max_width`/`max_height` reuse the existing bounded JPEG pipeline and only apply aspect-preserving downscale; they never upscale. The output separates `source_width`/`source_height` from encoded `width`/`height`, while global origin and native DPI mapping remain private.
...
Full-display observation receives separate authority from single-window observation because it widens the privacy surface. Windows and macOS share the same exact-display discovery, snapshot-generation, and bounded JPEG contract. Pointer actions remain Windows-only and depend on fresh snapshot generation, correct display-local geometry, DPI/display handling, and post-effect reconciliation. Windows and macOS clipboard read/write share the same separately scoped bounded global authority while retaining platform-native plain-text representations and effect boundaries.
