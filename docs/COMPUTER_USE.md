...
Full-display observation receives separate authority from single-window observation because it widens the privacy surface. Pointer actions depend on fresh snapshot generation, correct display-local geometry, DPI/display handling, and post-effect reconciliation. Windows and macOS clipboard read/write share the same separately scoped bounded global authority while retaining platform-native plain-text representations and effect boundaries.
