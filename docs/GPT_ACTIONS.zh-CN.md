...
该单一 Action 用于导入当前 ChatGPT 会话里的生成图片、用户上传文件、Code Interpreter 产物、PDF、zip、CSV、JSON、文本文件以及其它受支持的有界二进制 artifact。不要为图片、zip、PDF 分别新增 dedicated GPT Actions。

推荐生图保存流程：

1. GPT 在当前 ChatGPT 会话中使用内置 image generation 生成图片；
2. GPT 调用 `importConversationFilesToProject`，传入 `openaiFileIdRefs`、`project`，以及可选 `output_dir`，例如 `docs/assets` 或 `artifacts/imports`；
