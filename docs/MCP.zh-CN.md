...
`tools/list` 恰好包含以下十三个操作。这是 `webcodex run` 与 `webcodex share`
使用的 surface；没有 Connector context 的普通 hosted/self-hosted Server 默认暴露
`local_coding`（或显式的 `full_operator_runtime`）而不是这十三个操作：
...
8 秒后可能 quick-yield。在十三工具 Connector surface 上，用 `task_review` 的
