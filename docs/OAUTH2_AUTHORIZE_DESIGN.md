...
- No `/oauth/authorize` route in Phase 2e-0.
- No HTML login page, username/password flow, consent page, or third-party
  cookie session.
- No authorization code issuance in Phase 2e-0.
...
| `state` | No | Opaque client value. Return byte-for-byte as supplied on success and redirect errors. |
...
`invalid_request`. The handler must not parse or modify `state`.
...
8. On success, generate one plaintext `wc_oac_*` code, store only its SHA-256
   hash and metadata, and redirect once to the registered redirect URI.
...
- `state=<original state>` when `state` was provided
...
- `state` is returned verbatim; WebCodex does not parse, normalize, or trust
  it.
...
- `state` is preserved exactly on success and redirect errors.

## Metadata Gating

`/.well-known/oauth-authorization-server` must remain unexposed until
`/oauth/authorize` is implemented and the Phase 2e-1 tests pass.
...
Do not advertise authorization server metadata while the browser
authorization entry point is missing.
...
5. Implement post-redirect-trust validation for `response_type`, PKCE,
   `scope`, and `resource`.
6. Insert hashed authorization code metadata.
7. Redirect with plaintext `code` and verbatim `state`.
8. Add authorization server metadata only after endpoint tests pass.
