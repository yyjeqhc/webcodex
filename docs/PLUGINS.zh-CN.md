...
Native Tool Plugin 允许 WebPi Runner 把任意本地可执行程序变成工具。Plugin
...
Native Plugin 是**受信任的本地 executable**。WebPi 不会 sandbox、隔离、签名或把
...
只看 Runner 父进程的 PATH。敏感 WebPi 进程凭据会被过滤。
...
Server-global WebPi tool namespace，也不会被追加到外层 MCP `tools/list`。Plugin
可以定义 `safe_delete`、`runtime_status` 或其他合法 provider-local 名称；它们不需要
和 WebPi builtin、其他 Runner 或同一 Runner 的其他 provider 做全局避让。

唯一稳定的 model-facing 入口是一等 WebPi 工具 `plugin_tool`。它的 ToolSpec 走和
其他 WebPi 工具相同的 canonical metadata/registry 链路，schema 与 Runner 是否在线、
...
终止整个 disposable candidate process tree。WebPi 在 check 中绝不会调用 provider
...
以及有界、由 WebPi 生成的安全 `detail`，而不是把所有坏 Plugin 都变成
`plugin_tool isError=true`。Tool definition validation failure 还会带一个很小的
`diagnostic`：使用有限、稳定的 WebPi-defined code，并在安全时携带 validated tool name
以及 `inputSchema` 等有限 field label。diagnostic 只来自 WebPi protocol parser/validator；
...
`webpi plugin` 现在明确分成两条路径：`plugin init` 是纯本地 scaffold generator，不使用
...
`webpi runner status --profile <profile>`；Server 不会为此发现或返回 Runner-local config
...
`WEBPI_TOKEN`，再 `WEBPI_PAT`；当前进程 `WEBPI_TOKEN`，再 `WEBPI_PAT`。
`WEBPI_PAT` 只是 additive user/API CLI input alias，不改变 Server bootstrap
`WEBPI_TOKEN` 的配置语义；两个环境变量同时存在时仍优先原有 `WEBPI_TOKEN`。如果需要
专用 authoring PAT，复用已有 `webpi tokens create-local` / Server token management flow，
...
配置的 command 与 arguments。对于由 `webpi plugin init` 创建的 Plugin，还应确认已执行
...
可能产生 `plugin_eof`，WebPi 不会根据 raw stderr 或 opaque argv 推断具体原因。
...
本阶段刻意没有 `webpi plugin call`。raw `plugin_tool describe -> call` 仍是 canonical
invocation path，并继续拥有原有 binding、effect、retry 和 `OutcomeUnknown` 语义。

Phase 1 曾有意暂缓 `webpi plugin init`，直到 SDK 建立真实 external dependency contract。
这个前置条件现在已经满足：`@yyjeqhc/webcodex-plugin-sdk@0.1.0` 已通过 npm 公开分发，Phase 3
因此加入使用该**精确兼容版本**的本地 scaffold。生成项目不依赖 WebPi 源码 checkout。
...
executable。WebPi Server / Runner 不会因为 SDK 而要求 Node；只有主动选择这个 SDK 的
...
`webcodex-plugin-v1`。每个 request/response 各占一行 JSON。它是 WebPi Plugin
Protocol，不是 MCP。
...
`inputSchema` / `outputSchema` 使用 WebPi 明确定义的小型 profile，**不宣称**支持完整
...
`tools/call` 进入 provider connection 之前，WebPi 会先从 exact frozen catalog
...
WebPi 会 fail closed 并 retire 该 provider instance。
...
WebPi 不会把它 re-resolve 到新的同名 provider/tool，不会自动生成新 binding，也不会
...
- 使用 shared-key OAuth bridge 时，需要显式
  `webpi connect ... --auth oauth --oauth-local-plugins`；该 opt-in 只授予
...
consequential WebPi execution 相同的 Workflow Session guard 和 authority-mode
permission policy；WebPi 不会从 MCP transport identity 推断 Workflow Session。
