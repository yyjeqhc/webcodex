...
状态：Iteration 7 已完成人工评审、完整测试和最终 squash。Iteration 8.0
只实现 Product Entry and Golden Path Convergence 的第一个垂直切片；Browser IDE、
多语言 recipe、数据分析平台和 Iteration 8 总体 20% LOC 目标不在这一切片内。
...
| 本机 review/accept | `webcodex task show/accept/reject` 按同一 Git root/profile 打开 SQLite state | task/result id（用于精确人类决定） | `task_finish` 后一条本机 accept/reject | target checkout 漂移、patch precondition、runtime restart 后需 resume/reject | 保持现有 host-local authority | 无；这不是模型 capability，也不能移到 Hosted |
