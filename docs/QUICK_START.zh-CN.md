...
这是 WebCodex 推荐的 local-first 首次体验路径。先让一个 server、一个 agent、一个已注册项目，以及一个 MCP 或 GPT Action client 跑通，再决定生产部署方式。
...
保持这个进程运行。复制命令打印的 shared key。笔记和文档里使用 placeholder，不要把真实 token 值写入提交文件。
...
```bash
export WEBCODEX_KEY=<server up 打印的 shared key>
...
截图、token 说明和常见 MCP 错误见 [MCP.zh-CN.md](MCP.zh-CN.md)。
...
Action authentication 选择 Bearer/API-key auth，首次体验使用同一个 shared key。完整设置见 [GPT_ACTIONS.zh-CN.md](GPT_ACTIONS.zh-CN.md)。
...
- shared key 或 token 只用于对应 client。不要把 bootstrap、account 或 agent credential 粘贴到 MCP 或 GPT Actions。
...
首次体验时，server、agent connection、MCP client 或 GPT Action 使用同一个 shared key。managed deployment 中，MCP/GPT Actions 使用 `wc_pat_xxx`，agent 只使用 `wc_agent_xxx`。
