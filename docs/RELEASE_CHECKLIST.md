...
- Prefer `python3 scripts/release_operator.py doctor --version <VERSION> --source-sha <MAIN_COMMIT> --root <EXACT_MAIN_WORKTREE>` before release day. `doctor` is read-only: it combines publication preflight, the five-way release-version fence, exact-main daily-CI proof, the explicit/reusable extended-native workflow, the six-platform npm runtime, the primary Windows x64/ARM64 + macOS Apple-Silicon Desktop contract, the supplemental macOS Intel publication workflow, draft-capable authenticated Release-list access, verifier parsing, required tool availability, and `actionlint` when installed. It validates that rare native runners stay out of ordinary CI and that both macOS distribution paths remain ad-hoc signed without paid Apple signing credentials. It reports all failed checks together and performs no tag, dispatch, draft, publish, or deployment mutation.
...
- Build the six published runtime platforms (`linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`, `win32-x64`, `win32-arm64`) natively from the exact `v<VERSION>` tag through the reviewed release-build workflow. Both Windows jobs and the macOS Apple-Silicon job stage their already-built and verified runtime binaries into the matching primary Desktop distribution without recompiling a second runtime. `win32-arm64` uses the native Windows ARM64 runner, `darwin-x64` still uses GitHub's native Intel runner for the primary runtime archive, and `darwin-arm64` uses native Apple Silicon. Starting with `v0.4.3`, the Intel lane stops after its runtime archive in the primary build; the later `release-desktop-darwin-x64.yml` workflow uses that exact published archive as its Desktop input. Platform code signing may change Mach-O bytes copied into an `.app`, so native smoke/evidence records signed output identity rather than claiming byte-for-byte equality after signing.
...
- After all six native targets succeed, `release-build.yml` must aggregate only artifacts from that same workflow run and upload one `<ARCHIVE_STEM>-bundle`. Historical releases through `v0.4.2` retain the four-Desktop primary contract. Starting with `v0.4.3`, the primary bundle contains the six unchanged native archives, both Windows Desktop installers, the macOS Apple-Silicon DMG, `manifest.json`, one immutable `SHA256SUMS` covering those nine primary product bytes, both Linux ELF reports, and `release-build.json`. `release-build.json` keeps all six runtime entries under `artifacts` and the three primary Desktop entries under `desktop_artifacts`; the npm manifest remains six-platform runtime-only. The later Intel Desktop DMG is deliberately absent from this bundle and from the primary `SHA256SUMS`.
...
- Create a draft GitHub Release from exactly the retained primary bundle product assets. For `v0.4.3+`, that means the six native archives, both Windows Desktop installers, the macOS Apple-Silicon DMG, and the primary `SHA256SUMS`; do not wait for or pre-create the Intel Desktop DMG. Before making the Release public, run `python3 scripts/release_operator.py verify-draft --bundle-dir <BUNDLE_DIR>` and require the primary draft asset set, sizes, states, and GitHub-provided `sha256:` digests to match retained bytes. Do not re-download the same retained draft bytes on the control host. The post-public verifier hashes Desktop distributions but does not claim to prove macOS code signing/notarization.
...
- Starting with `v0.4.3`, macOS Intel Desktop publication is owned by `.github/workflows/release-desktop-darwin-x64.yml`, also triggered only after the GitHub Release is public (or by guarded manual backfill from reviewed `main`). It downloads the exact published `darwin-x64` runtime archive and primary `SHA256SUMS`, verifies release provenance and runtime identity on the native Intel runner, builds/smokes the ad-hoc signed DMG, then attaches `webcodex-desktop-v<VERSION>-darwin-x64.dmg` plus its dedicated `.sha256` sidecar. It never rewrites `SHA256SUMS`. On rerun, each existing supplemental asset must match byte-for-byte; a missing counterpart may be added, while conflicting or duplicate bytes fail closed.
...
3. After readiness success, the plan stops at `awaiting_tag_authorization`. Only after explicit human authorization create and push the immutable annotated `v<VERSION>` tag at that exact source; never move it. On the next resume the plan reconciles the remote annotated tag and dispatches/observes one bound authoritative build. The low-level equivalent is `build-start --source-sha <TAG_COMMIT> --tag v<VERSION> --state-file <BUILD_STATE>` once, then `build-status --state-file <BUILD_STATE> --wait-secs 7200`. Require all six native runtime targets, the version-appropriate primary Desktop candidates/smokes, native macOS primary signing evidence, and `assemble` to succeed in that bound workflow run. For `v0.4.3+`, Intel Desktop is not part of this primary build gate.
...
6. The plan then stops at `awaiting_draft_authorization`. After explicit authorization, create a **draft** GitHub Release from the version-appropriate retained primary assets; for `v0.4.3+`, upload the six archives, the three primary Desktop distributions, `SHA256SUMS`, and `webcodex-release-manifest.json` when the retained bundle includes that runtime metadata asset. The next resume runs the same `verify-draft` digest gate before advancing to the publication authorization boundary.
...
8. Require `.github/workflows/release-image.yml` to finish successfully for the same public `v<VERSION>` Release. It must publish/reconcile `ghcr.io/yyjeqhc/webcodex-server:v<VERSION>` for both Linux architectures, attach the digest metadata plus the self-contained clone-free bootstrap, verify its public SHA-256 and embedded pinned-digest contract, and pass its anonymous GHCR availability gate. For `v0.4.3+`, `release-desktop-darwin-x64.yml` may run independently in parallel: it is intentionally non-blocking for the core Release, and a failed/slow Intel DMG can be retried or manually backfilled against the same immutable public tag without changing primary assets.
9. From one trusted, well-connected Linux verification host, run `python3 scripts/verify_public_release.py <VERSION>`. This remains the full public-byte acceptance pass for npm plus the six native GitHub archives and the primary Desktop distributions required by that release generation: none before `0.4.0`, three for `0.4.0`/`0.4.1`, four for `0.4.2`, and three primary Desktop assets from `0.4.3+`. For `v0.4.3+`, absent Intel supplemental assets do not fail core verification; a one-file intermediate state is reported as `publication_in_progress`, and once both DMG and dedicated checksum sidecar exist the verifier checks the pair. The verifier cross-checks npm manifest / primary `SHA256SUMS` / GitHub asset digests, validates archive membership and foreign binary architecture without executing foreign binaries, hashes bounded public Desktop bytes, and rechecks Linux GLIBC/`DT_NEEDED` metadata. Native Mac workflows continue to own ad-hoc `codesign` and DMG execution evidence.
