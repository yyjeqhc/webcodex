...
- The release control host downloads that single assembled bundle instead of fetching five target artifacts independently. Verify `release-build.json` matches the immutable tag/source and recorded workflow run, then verify `SHA256SUMS` before publication. Never substitute an archive from another workflow run.
...
4. Download only that run's `<ARCHIVE_STEM>-bundle` to the release control host. Verify `release-build.json`, `SHA256SUMS`, archive set, source SHA, shared `built_at`, and workflow run id. Create a draft GitHub Release and upload the five exact archives plus `SHA256SUMS`; download those draft assets once and verify SHA-256 against the retained bundle bytes.
