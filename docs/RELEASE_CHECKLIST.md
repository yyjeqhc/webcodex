...
- macOS verification candidates use ad-hoc signing and are explicitly not notarized. A formal immutable release must require `Developer ID Application` signing plus Apple notarization and stapling; missing release credentials fail the native Mac release-build job rather than falling back to ad-hoc signing. The App Store Connect API private key exists only as a temporary mode-0600 file under the CI runner's temporary directory and is never uploaded as an artifact.
