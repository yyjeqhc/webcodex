...
3. Build the four native artifacts from that exact tag on the dedicated Special compatibility release builder (`linux-x64`, glibc 2.17 baseline), Orb (`linux-arm64`), Mini (`darwin-arm64`), and MSI (`win32-x64`). Verify version/build identity on each host, enforce the Linux x64 `GLIBC_* <= 2.17` gate, and transfer the final archives back to OE.
