...
The readiness workflow uses a fail-fast two-stage graph. Stage 1 is the complete test gate: the canonical `scripts/release_check.sh`, the complete locked Rust workspace suite (package-sharded without test-name filters), frontend typecheck/tests/committed-build validation, WebSocket and polling zero-config E2E, and coding-loop compare eval all start in parallel. The eval job explicitly builds debug Server/Runner fixtures before running the harness so cold-runner Rust compilation is not misclassified as a service-start timeout. Stage 2 contains only expensive release-surface validation and cannot start until every Stage-1 job succeeds: native release-profile gates for all six published platforms plus both disposable Server-image architectures. Linux x64/arm64 use the same native manylinux2014 userspaces and ABI/dependency checks as formal release builds; macOS x64/arm64 each build and package natively on the matching GitHub-hosted architecture and run the Runner suite; Windows x64/arm64 perform native release builds, PE architecture checks, packaging, and the local npm-install smoke. These pre-tag artifacts are disposable and are never uploaded or promoted. `release_check.sh` includes formatting, workspace all-target check, focused metadata/schema/OpenAPI/MCP tests, release-tooling self-tests, Markdown local-link validation, and static current-contract/leakage guards.
...
As part of Stage 1, the exact-source release-readiness workflow builds dedicated debug `webcodex-server` / `webcodex-runner` test fixtures once for both supported zero-config transports while frontend, eval, release-contract, and Rust-suite jobs run in parallel. Native release-profile and Server-image validation do not start until that complete test gate succeeds. None of those binaries or archives are uploaded or treated as release candidates; `.github/workflows/release-build.yml` remains the only formal candidate producer after the immutable tag exists.
...
- Build the six published platforms (`linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`, `win32-x64`, `win32-arm64`) natively from the exact `v<VERSION>` tag through the reviewed release-build workflow. `darwin-x64` uses GitHub's Intel macOS runner rather than cross-compiling on Apple Silicon. Do not rebuild an artifact on an intermediate packaging machine or substitute a cross-compiled artifact for native validation.
...
- After all six native targets succeed, the release-build workflow must aggregate only artifacts from that same workflow run, verify every per-target SHA sidecar, and upload one `<ARCHIVE_STEM>-bundle`. For a real `v<VERSION>` release the bundle contains the six unchanged native archives, `manifest.json`, `SHA256SUMS`, both Linux ELF reports, and `release-build.json`; the canonical `scripts/prepare_release_metadata.py` still generates the publish metadata, but it now runs inside this same-run assemble job.
...
2. Run `release_operator.py readiness-start` with a fresh durable state file, then `readiness-status --wait-secs 3600` on that same state. Require terminal success for the exact source: Stage 1 must pass the canonical release contract, complete package-sharded locked Rust suite, frontend, E2E, and eval in parallel before Stage 2 may fan out any native release-profile or Server-image build. If dispatch/observation is uncertain, recover from the same state instead of blind redispatch.
3. After explicit human authorization, create and push the immutable annotated `v<VERSION>` tag at that exact source; never move it. Then run `release_operator.py build-start --source-sha <TAG_COMMIT> --tag v<VERSION> --state-file <BUILD_STATE>` once, and `build-status --state-file <BUILD_STATE> --wait-secs 7200` on the same state until terminal. Require all six native targets plus `assemble` to succeed in that one bound workflow run.
