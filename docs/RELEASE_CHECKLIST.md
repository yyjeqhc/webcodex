...
The readiness workflow uses a fail-fast two-stage graph. Stage 1 runs the canonical `scripts/release_check.sh` in parallel with the complete locked Rust workspace suite, package-sharded without test-name filters. Native release-profile and Server-image builds are downstream of both Stage-1 gates, so a Rust or release-contract failure is discovered before expensive artifact compilation starts. After Stage 1 succeeds, Stage 2 fans out frontend typecheck/tests/committed-build validation, WebSocket and polling zero-config E2E, coding-loop compare eval, and native release-profile gates for all five published platforms plus both disposable Server-image architectures. Linux x64/arm64 use the same native manylinux2014 userspaces and ABI/dependency checks as formal release builds; macOS arm64 performs a release build/package plus the Runner suite; Windows x64/arm64 perform native release builds, PE architecture checks, packaging, and the local npm-install smoke. These pre-tag artifacts are disposable and are never uploaded or promoted. `release_check.sh` includes formatting, workspace all-target check, focused metadata/schema/OpenAPI/MCP tests, release-tooling self-tests, Markdown local-link validation, and static current-contract/leakage guards.
...
After the Stage-1 correctness gates succeed, the exact-source release-readiness workflow builds dedicated debug `webcodex-server` / `webcodex-runner` test fixtures once for both supported zero-config transports while independent frontend, eval, native release-profile, and Server-image validation lanes run in parallel. None of those binaries or archives are uploaded or treated as release candidates; `.github/workflows/release-build.yml` remains the only formal candidate producer after the immutable tag exists.
...
2. Run `release_operator.py readiness-start` with a fresh durable state file, then `readiness-status --wait-secs 3600` on that same state. Require terminal success for the exact source: Stage 1 must pass the canonical release contract plus the complete package-sharded locked Rust suite before Stage 2 may fan out frontend/E2E/eval and all native/image gates. If dispatch/observation is uncertain, recover from the same state instead of blind redispatch.
