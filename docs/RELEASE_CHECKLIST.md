...
- Build the six published runtime platforms (`linux-x64`, `linux-arm64`, `darwin-x64`, `darwin-arm64`, `win32-x64`, `win32-arm64`) natively from the exact `v<VERSION>` tag through the reviewed release-build workflow. The same `win32-x64` job also stages its already-built and verified `webcodex.exe`, `webcodex-server.exe`, and `webcodex-runner.exe` bytes into the one Windows x64 NSIS Desktop installer. `darwin-x64` uses GitHub's Intel macOS runner rather than cross-compiling on Apple Silicon. Do not rebuild an artifact on an intermediate packaging machine or substitute a cross-compiled artifact for native validation.
...
- After all six native targets succeed, the release-build workflow must aggregate only artifacts from that same workflow run, verify every per-target SHA sidecar plus the Desktop installer sidecar, and upload one `<ARCHIVE_STEM>-bundle`. For a real `v<VERSION>` release the bundle contains the six unchanged native archives, `webcodex-desktop-v<VERSION>-win32-x64-setup.exe`, `manifest.json`, one `SHA256SUMS` covering all seven downloadable product bytes, both Linux ELF reports, and `release-build.json`. `release-build.json` keeps the six runtime entries under `artifacts` and the installer separately under `desktop_artifacts.win32-x64`; the canonical `scripts/prepare_release_metadata.py` still generates npm manifest/checksum metadata inside this same-run assemble job, and the npm manifest remains six-platform only.
...
- Create a draft GitHub Release and upload exactly the six native archives, the Windows x64 Desktop installer, and `SHA256SUMS` (eight initial assets). Before making it public, run `python3 scripts/release_operator.py verify-draft --bundle-dir <BUNDLE_DIR>`; require the draft asset set, sizes, states, and GitHub-provided `sha256:` digests to match the retained bundle. Do not re-download the same retained draft bytes on the control host. The post-public `verify_public_release.py` run is the single full public-byte download/verification pass.
...
9. From one trusted, well-connected Linux verification host, run `python3 scripts/verify_public_release.py <VERSION>`. This remains the single full public-byte acceptance pass for npm plus the six native GitHub archives and, for `0.4.0+`, the Windows x64 Desktop installer; pre-0.4 historical releases remain valid without Desktop. The verifier cross-checks npm manifest / `SHA256SUMS` / GitHub asset digests, validates archive membership and foreign binary architecture without executing foreign binaries, verifies the bounded public Desktop installer bytes by SHA-256, and rechecks Linux GLIBC/`DT_NEEDED` metadata. The container workflow independently proves the public GHCR digest/platform contract. Native per-OS public installs are targeted diagnostics; the authoritative Desktop install/run/uninstall proof belongs to Windows CI/release-build. Then clean disposable publication worktrees and prefixes while retaining the versioned release audit bundle and durable readiness/build state files.
