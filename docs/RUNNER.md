...
| **profile** | A named local client configuration (`runner.toml`, tokens, paths). |
...
`runner.toml` is the canonical Runner config filename for 0.4. New `runner init`,
`login`, `connect`, profile, and service layouts use it. A pre-0.4 directory
containing only legacy `agent.toml` remains readable and continues to use that
file in place; WebCodex does not create a second copy. If both `runner.toml` and
`agent.toml` exist in the same authoritative config directory, WebCodex fails
closed and requires the operator to resolve the ambiguity. With neither file,
initialization targets `runner.toml`.
