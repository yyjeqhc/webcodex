...
function runtimeCollaborationMessageSides(messages) {
    const byId = new Map();
...
function closeRuntimeInspector(restoreFocus = false) {
    if (el("runtime-console")?.classList.contains("context-docked"))
        return;
    const inspector = document.querySelector(".runtime-inspector");
    if (!inspector?.open)
        return;
    inspector.open = false;
    if (restoreFocus)
        inspector.querySelector(".context-trigger")?.focus();
}
...
function syncResponsiveNavigation() {
    ...
    if (contextDocked && inspector)
        inspector.open = true;
    else if (wasContextDocked && inspector)
        inspector.open = false;
...
function renderCollaboration(statusText, consumeMutationNotice = true) {
    ...
    const appendMessage = (message, depth, parentUnavailable) => {
        ...
        authorName.textContent = messageSide === "incoming" ? tr("Agent") : tr("You");
