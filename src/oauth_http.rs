...
// ---------------------------------------------------------------------------
// OAuth2 error helpers
// ---------------------------------------------------------------------------

/// Render an OAuth2 error response (RFC 6749 §5.2).
...
pub(crate) async fn oauth_token(req: &mut Request, depot: &mut Depot, res: &mut Response) {
    ...
    let code_record = match db.consume_oauth_authorization_code_by_hash(&code_hash, now) {
        Ok(Some(r)) => r,
        Ok(None) => {
            oauth_error(
                res,
    ...
    if let Err(e) = db.insert_oauth_access_token(&at_record) {
        tracing::warn!("failed to insert oauth access token: {}", e);
        oauth_error(
            res,
            StatusCode::INTERNAL_SERVER_ERROR,
            "server_error",
    ...
    if let Err(e) = db.insert_oauth_refresh_token(&rt_record) {
        tracing::warn!("failed to insert oauth refresh token: {}", e);
        oauth_error(
            res,
            StatusCode::INTERNAL_SERVER_ERROR,
            "server_error",
