use super::*;
use crate::shell_protocol::{ShellJobValidationMetadata, ShellJobValidationStep};

#[tokio::test]
async fn validation_job_rechecks_cargo_test_count_assertion_capability_after_downgrade() {
    let registry = ShellClientRegistry::default();
    let capabilities = |count_assertion| ShellClientCapabilities {
        jobs: true,
        async_shell_jobs: true,
        structured_validation_argv: true,
        structured_cargo_test_count_assertion: count_assertion,
        ..Default::default()
    };
    let register = |capabilities| ShellClientRegisterRequest {
    ...
    registry
        .register(register(capabilities(true)))
        .await
        .unwrap();
    assert!(registry
        .client_supports(
            "cargo-assertion-fence",
            crate::shell_protocol::SHELL_CLIENT_CAPABILITY_STRUCTURED_CARGO_TEST_COUNT_ASSERTION,
        )
        .await
        .unwrap());

    // Same Runner identity re-registers without the additive durability
    // capability after an earlier observer saw it as enabled. Job admission
    // must use the current registration at its linearization point.
    registry
        .register(register(capabilities(false)))
        .await
        .unwrap();
    ...
    let error = registry
        .start_job_with_metadata(
            ShellJobOpRequest {
                op: "start".to_string(),
                client_id: Some("cargo-assertion-fence".to_string()),
        ...
        .await
        .unwrap_err();
    assert!(error.contains("structured_cargo_test_count_assertion_unavailable"));
    assert!(registry.list_jobs(Some(10)).await.is_empty());
    let inner = registry.inner.lock().await;
    assert!(inner
        .queues_by_client
        .get("cargo-assertion-fence")
