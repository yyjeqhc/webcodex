...
pub(crate) fn builtin_coding_workflow_projection() -> Value {
    json!({
        ...
        "model_protocol": {
            ...
            "session_message_resolution": "After a non-todo Session message is already handled, a stateless MCP schema may expose session_message_resolution. Piggyback {message_id, resolution} on the next ordinary WebCodex call together with recording_session_id instead of making a separate resolve call. For requires_ack guidance, echo the same id in ack_session_message_ids on that request. The target is always the exact recording Session; the wrapper field is removed before concrete tool parsing. Do not use it to predict the success of the current tool call. Todo completion still uses complete_session_message.",
