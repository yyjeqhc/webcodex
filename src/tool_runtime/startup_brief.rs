...
pub(crate) fn builtin_coding_workflow_projection(profile: CodingGuidanceProfile) -> Value {
    json!({
        ...
        "model_protocol": {
            "goal_workflow": "On exact Session re-entry, honor work_on_project.goal_context: reuse one exact active Goal with get_goal/present_goal_plan; choose explicitly among multiple candidates; never infer from Project/Window/title/recency. For ordinary new substantial multi-step/cross-turn work with no reusable Goal, call prepare_goal_workflow with the exact current Workflow Session and bounded completion_conditions/steps, then present_goal_plan. available=false never proves no Goal. Host continuation setup/readiness remains separate. Low-level create_goal and associate_goal_workflow_session remain available for advanced composition. Tiny one-step lookups/trivial edits need no Goal. This applies independently of AGENTS.md.",
