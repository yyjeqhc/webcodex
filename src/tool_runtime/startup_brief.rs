...
pub(crate) fn builtin_coding_workflow_projection(profile: CodingGuidanceProfile) -> Value {
    json!({
        ...
        "model_protocol": {
            "goal_workflow": "For ordinary new substantial multi-step/cross-turn development, fixes, refactoring, deployment, troubleshooting or migration needing validation/review, call prepare_goal_workflow with the exact current Workflow Session plus bounded completion_conditions/steps and optional explicit controller Agent, then present_goal_plan. This atomically admits new durable Goal + Session correlation only; Host continuation setup/readiness remains separate. Low-level create_goal and associate_goal_workflow_session remain available for explicit advanced composition or an already-known Goal. Tiny one-step lookups/trivial edits do not need a Goal. This WebCodex workflow applies across repositories, independently of AGENTS.md.",
