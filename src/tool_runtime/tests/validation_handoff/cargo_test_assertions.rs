...
#[tokio::test]
async fn agent_job_cargo_test_assertion_requires_durable_runner_capability() {
    let client_id = "vhandoff-assertion-capability";
    let runtime = runtime_with_agent_project(client_id)
        .with_validation_sync_wait(std::time::Duration::from_millis(50));
    register_agent(
        &runtime,
    ...
    assert_eq!(short.output["exit_code"], 0);
    assert_eq!(short.output["failure_kind"], "validation_failed");
    assert_eq!(
    ...
    assert!(
        !long.success,
        "a Job-capable timeout must not bypass the durable metadata fence"
    );
    assert_eq!(long.output["failure_kind"], "capability_unavailable");
    assert_eq!(long.output["execution_state"], "not_started");
    assert_eq!(long.output["command_started"], false);
